...
import { parse, table } from './types.js';
import {
  CffEncoding, cffStandardEncoding, cffExpertEncoding, cffStandardStrings,
} from './encoding.js';
import { GlyphSet, cffGlyphLoader } from './font.js';
...
function entriesToObject(entries) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if (o.hasOwnProperty(key) && !Number.isNaN(o[key])) {
...
export function parseCFFTable(data, start, font) {
  font.tables.cff = {};
...
function makeCharsets(glyphNames, strings) {
  // Format byte + 2 bytes per glyph SID
  const data = new Array(1 + glyphNames.length * 2);
  data[0] = 0; // format = 0
  for (let i = 0; i < glyphNames.length; i += 1) {
    const glyphSID = encodeString(glyphNames[i], strings);
    const off = 1 + i * 2;
    data[off] = (glyphSID >> 8) & 0xFF;
    data[off + 1] = glyphSID & 0xFF;
  }

  return new table.Record('Charsets', [
    { name: 'data', type: 'LITERAL', value: data },
  ]);
}

// Encode a CFF NUMBER operand directly to bytes in the output array.
function pushNumber(d, v) {
  if (v >= -107 && v <= 107) {
    d.push(v + 139);
  } else if (v >= 108 && v <= 1131) {
    v -= 108;
    d.push((v >> 8) + 247, v & 0xFF);
  } else if (v >= -1131 && v <= -108) {
    v = -v - 108;
    d.push((v >> 8) + 251, v & 0xFF);
  } else if (v >= -32768 && v <= 32767) {
    d.push(28, (v >> 8) & 0xFF, v & 0xFF);
  } else {
    d.push(29, (v >> 24) & 0xFF, (v >> 16) & 0xFF, (v >> 8) & 0xFF, v & 0xFF);
  }
}

// Encode a glyph directly to CFF charstring bytes, bypassing intermediate objects.
function encodeGlyphToBytes(glyph) {
  const d = [];
  const path = glyph.path;
  pushNumber(d, glyph.advanceWidth);
  let x = 0;
  let y = 0;
  for (let i = 0; i < path.commands.length; i += 1) {
    ...
    if (cmd.type === 'M') {
      pushNumber(d, Math.round(cmd.x - x));
      pushNumber(d, Math.round(cmd.y - y));
      d.push(21); // rmoveto
      x = Math.round(cmd.x);
      y = Math.round(cmd.y);
    } else if (cmd.type === 'L') {
      ...
      pushNumber(d, Math.round(cmd.x1 - x));
      pushNumber(d, Math.round(cmd.y1 - y));
      pushNumber(d, Math.round(cmd.x2 - cmd.x1));
      pushNumber(d, Math.round(cmd.y2 - cmd.y1));
      pushNumber(d, Math.round(cmd.x - cmd.x2));
      pushNumber(d, Math.round(cmd.y - cmd.y2));
      d.push(8); // rrcurveto
      x = Math.round(cmd.x);
      y = Math.round(cmd.y);
    }
...
function makeCharStringsIndex(glyphs) {
  ...
  for (let i = 0; i < glyphs.length; i += 1) {
    ...
    const bytes = encodeGlyphToBytes(glyph);
    t.charStrings.push({ name: glyph.name, type: 'LITERAL', value: bytes });
  }
...
export function makeCFFTable(glyphs, options) {
